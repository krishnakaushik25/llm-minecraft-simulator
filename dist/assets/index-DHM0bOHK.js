...
var _a;
import { V as Vector3, E as Euler, R as Raycaster, a as Vector2, S as Scene, C as Color, P as PerspectiveCamera, W as WebGLRenderer, b as PCFSoftShadowMap, A as AmbientLight, D as DirectionalLight, B as BoxGeometry, M as MeshBasicMaterial, c as Mesh, d as Matrix4, e as MeshLambertMaterial } from "./vendor-Bcw4N6We.js";
import { C as CreateMLCEngine } from "./llm-DFcpMLFG.js";
(function polyfill() {
...
class Game {
  constructor() {
    ...
    this.keys = {};
    this.camera_rotation = { x: 0, y: 0, rotationSpeed: 0.05 };
    this.mouse = { x: 0, y: 0 };
    ...
    this.world = {
      ...
      // Store blocks by position key "x,y,z"
      chunkSize: 16,
      worldHeight: 32
    };
    this.blockTypes = {
      grass: { color: 5025616, name: "Grass", rarity: "common" },
      stone: { color: 7697781, name: "Stone", rarity: "common" },
      wood: { color: 9268835, name: "Wood", rarity: "common" },
      sand: { color: 16771899, name: "Sand", rarity: "common" },
      water: { color: 2201331, name: "Water", rarity: "common", transparent: true },
      coal: { color: 2171169, name: "Coal Ore", rarity: "uncommon" },
      iron: { color: 12434877, name: "Iron Ore", rarity: "uncommon" },
      gold: { color: 16766720, name: "Gold Ore", rarity: "rare" },
      diamond: { color: 48340, name: "Diamond Ore", rarity: "rare" },
      obsidian: { color: 1710618, name: "Obsidian", rarity: "rare" },
      ice: { color: 14743295, name: "Ice", rarity: "uncommon", transparent: true },
      leaves: { color: 3046706, name: "Leaves", rarity: "common", transparent: true },
      lava: { color: 16733986, name: "Lava", rarity: "rare", glowing: true },
      snow: { color: 16777215, name: "Snow", rarity: "uncommon" },
      dirt: { color: 9268835, name: "Dirt", rarity: "common" }
    };
    ...
    this.worldGen = {
      ...
      // Reduced from 8
      frequency: 0.03,
      // Increased for less detail but better performance
      chunkSize: 8,
      // Reduced from 16 for faster generation
      renderDistance: 2,
      // Reduced from 3 for better performance
      loadedChunks: /* @__PURE__ */ new Map(),
      heightMap: /* @__PURE__ */ new Map(),
      biomes: ["plains", "forest", "desert", "mountains", "ocean"],
      maxChunksPerFrame: 1,
      // Limit chunk generation per frame
      lastChunkCheck: 0
      // Throttle chunk checking
    };
  ...
  generateChunk(chunkX, chunkZ) {
    ...
    for (let x = startX; x < startX + this.worldGen.chunkSize; x++) {
      for (let z = startZ; z < startZ + this.worldGen.chunkSize; z++) {
        ...
        for (let y = Math.max(-5, height - 3); y <= height; y++) {
          let blockType = "stone";
          if (y === height) {
  ...
  getSurfaceBlock(biome, height) {
    switch (biome) {
      case "desert":
        return "sand";
      case "ocean":
        return height < -2 ? "water" : "sand";
      case "mountains":
        return height > 5 ? "snow" : "stone";
      case "forest":
        return "grass";
      default:
        return "grass";
  ...
  gameLoop() {
    ...
    if (currentTime - this.performance.lastFpsUpdate > 1e3) {
      this.performance.fps = Math.round(this.performance.frameCount * 1e3 / (currentTime - this.performance.lastFpsUpdate));
  ...
  updatePlayer() {
    ...
    if (!this.player.onGround) {
      this.player.velocity.y -= 25 * delta;
    } else {
  ...
  addBlock(x, y, z, type = "grass") {
    ...
    const block2 = new Mesh(geometry, material);
  ...
  updateWorldGeneration() {
    ...
    const maxDistance = this.worldGen.renderDistance + 3;
